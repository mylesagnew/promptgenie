...
@click.option("--system", default=None, help="System prompt text (prepended before the prompt).")
@click.option(
    "--concurrency",
    "-j",
    default=4,
...
@click.option(
    "--timeout", default=60, type=int, show_default=True, help="Per-model timeout in seconds."
)
@click.option(
    "--max-tokens", default=1024, type=int, show_default=True, help="Max output tokens per model."
)
@click.option(
    "--determinism-runs",
    default=1,
...
@click.option(
    "--fail-on-regression", is_flag=True, help="Exit 1 if comparison finds any metric regression."
)
@click.option(
...
@click.option("--changed", is_flag=True, help="Only run if this file is in the git-changed set.")
@click.option(
    "--base-ref",
    default="origin/main",
    show_default=True,
...
def evaluate_cmd(
    ...
    if changed and file != "-":
        ...
        if Path(file).resolve() not in changed_resolved:
            diag_console.print(f"[dim]Skipping {file!r} — not in changed set (--changed).[/dim]")
    ...
    if not model_specs:
        diag_console.print(
            "[red]Error:[/red] No models specified. Use --models claude,gpt-4.1 or --model <spec>."
        )
    ...
    if compare_baseline_name:
        ...
        if baseline is None:
            ...
            regression_report = compare_to_baseline(
                matrix_result, baseline, thresholds, current_scan_risk=current_risk
            )
    ...
    if reporter.active or summary_path:
        md = format_matrix_summary(
            matrix_result, regression_report=regression_report, prompt_file=file
        )
...
def _print_regression_report(report: object) -> None:
    ...
    baseline_name = report.baseline_name  # type: ignore[attr-defined]

    if regs:
        console.print(
            f"\n[red bold]⚠ {len(regs)} regression(s) vs baseline {baseline_name!r}:[/red bold]"
        )
        for r in regs:
...
def _output_sarif(matrix_result: object, regression_report: object | None, file_path: str) -> None:
    results = matrix_result.results  # type: ignore[attr-defined]
    sarif_results = []
    rules = []
        ...
        "runs": [
            {
                ...
                "properties": {
                    ...
                    "has_regressions": (
                        regression_report.has_regressions if regression_report else False  # type: ignore[attr-defined]
                    ),
                },
            }
        ],
