...
def template_show_cmd(template_id: str, output_format: str) -> None:
    ...
    if t.variables:
        ...
        for v in t.variables:
            req = " [red](required)[/red]" if v.required else f" [dim](default: {v.default!r})[/dim]"
...
def template_validate_cmd(template_id: str) -> None:
    ...
    if errors:
        console.print(f"[red]✗[/red] Template [bold]{template_id}[/bold] has {len(errors)} error(s):")
        for e in errors:
...
def template_new_cmd(template_id: str, name: str, category: str, layer: str) -> None:
    ...
    if layer == "project":
        path = save_project_template(record)
    else:
        path = save_user_template(record)
...
def template_edit_cmd(template_id: str) -> None:
    ...
    if ret == 0:
        ...
        if t2:
            ...
            if errors:
                console.print(f"[yellow]Warning:[/yellow] Template has validation errors:")
                for e in errors:
