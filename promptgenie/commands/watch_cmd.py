...
@click.option("--run", "pipelines", multiple=True,
              type=click.Choice(["lint", "scan", "policy"], case_sensitive=False),
              default=["lint", "scan"],
              help="Pipelines to run on each change (repeatable). Default: lint scan.")
@click.option("--debounce", default=300, type=int, show_default=True,
              help="Debounce delay in milliseconds (requires watchfiles).")
@click.option("--fail-on-policy/--no-fail-on-policy", default=True, show_default=True,
