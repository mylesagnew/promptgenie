...
def _call_model(client, model: str, prompt: str, system: Optional[str] = None) -> tuple[str, dict]:
...
Score the response on all six dimensions and return the JSON object."""
    ...
    if not match:
        return {d: 50 for d in RUBRIC_DIMENSIONS}, "Could not parse judge response."

    try:
        parsed = json.loads(match.group())
    except json.JSONDecodeError:
        return {d: 50 for d in RUBRIC_DIMENSIONS}, "Invalid JSON from judge."
...
def compare_benchmarks(runs_a: list[BenchmarkRun], runs_b: list[BenchmarkRun]) -> dict:
    ...
    def avg(runs: list[BenchmarkRun]) -> dict:
        ...
        return {
            ...
            "scores": {d: int(sum(r.rubric_scores.get(d, 0) for r in runs) / len(runs)) for d in dims},
