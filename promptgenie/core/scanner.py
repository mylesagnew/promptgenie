...
SCAN_RULES: list[ScanRule] = [
    ...
    ScanRule(
        ...
        risk="HIGH",
        confidence="HIGH",
        message="JWT token detected in prompt.",
        recommendation="Remove JWT tokens from prompts. Tokens grant bearer access and must not be sent to LLM providers.",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_DB_URL",
        category="data-leakage",
        pattern=r"(postgres|postgresql|mysql|mongodb|redis|mssql|sqlserver|oracle|jdbc)://[^\s\"'<>]{8,}",
        risk="HIGH",
        confidence="HIGH",
        message="Database connection string detected.",
        recommendation="Remove connection strings from prompts. Use variable references instead.",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_INTERNAL_HOST",
        category="data-leakage",
        pattern=r"\b[a-z0-9][a-z0-9\-]*\.(internal|corp|local|intranet|lan|priv|private)\b",
        risk="MEDIUM",
        confidence="MEDIUM",
        message="Internal hostname detected — may expose internal network topology.",
        recommendation="Replace internal hostnames with generic placeholders before sending to external providers.",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_EMAIL",
        ...
        risk="LOW",
        confidence="MEDIUM",
        message="Email address detected — possible PII in prompt.",
        recommendation="Redact or anonymise email addresses before sending to external providers.",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_PHONE",
        ...
        risk="LOW",
        confidence="LOW",
        message="Phone number pattern detected — possible PII in prompt.",
        recommendation="Redact or anonymise phone numbers before sending to external providers.",
        false_positive_note="Date strings and other numeric patterns may trigger this rule.",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_CC",
        ...
        recommendation="Remove payment card data from prompts. This may violate PCI-DSS.",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_SSN",
        ...
        recommendation="Remove SSNs from prompts. This is regulated PII under HIPAA/GDPR.",
        false_positive_note="Date strings and other numeric patterns may match this format.",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_IPADDR",
        ...
        risk="MEDIUM",
        confidence="HIGH",
        message="Private IP address detected — may expose internal network topology.",
        recommendation="Replace internal IP addresses with generic placeholders before sending to external providers.",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_BEARER",
        ...
        risk="HIGH",
        confidence="HIGH",
        message="Bearer token detected in prompt.",
        recommendation="Remove bearer tokens from prompts. Use variable references or environment bindings instead.",
        use_original_text=True,
    ),
    ...
    ScanRule(
        id="LEAK_CUSTOMER_ID",
        category="data-leakage",
        pattern=r"\bcus_[A-Za-z0-9]{14,}\b",
        risk="MEDIUM",
        confidence="HIGH",
        message="Stripe customer ID detected — possible customer data in prompt.",
        recommendation="Replace customer IDs with anonymised references before sending to external providers.",
        false_positive_note="Only matches Stripe-format customer IDs (cus_…).",
        use_original_text=True,
    ),
    ScanRule(
        id="LEAK_SALESFORCE_ID",
        category="data-leakage",
        pattern=r"\b[0-9A-Za-z]{15}(?:[0-9A-Za-z]{3})?\b",
        risk="LOW",
        confidence="LOW",
        message="Possible Salesforce record ID detected (15 or 18 character alphanumeric).",
        recommendation="Verify this is not a real Salesforce ID before sending to external providers.",
        false_positive_note="Short alphanumeric strings (API keys, hashes) frequently match this pattern.",
        use_original_text=False,
    ),
    ScanRule(
        id="LEAK_UUID_CUSTOMER",
        category="data-leakage",
        pattern=r"(?:customer[_-]?id|user[_-]?id|account[_-]?id|tenant[_-]?id)\s*[:=]\s*['\"]?[0-9a-fA-F\-]{32,}",
        risk="MEDIUM",
        confidence="MEDIUM",
        message="Customer/user/account ID binding detected — may expose PII or internal identifiers.",
        recommendation="Replace explicit ID bindings with anonymised references or variable placeholders.",
        use_original_text=True,
    ),
    # Prompt injection
    ScanRule(
