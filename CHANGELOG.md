...
- **Prompt registry — remote OCI backend (Phase 6, Phase B.1).** `promptgenie registry` now pushes and pulls against any OCI-compatible registry (ghcr.io, Zot, Harbor, …) over HTTPS, in addition to the local store. `--remote host[/namespace]` on `push`/`pull`/`show`/`verify`/`tags` (a host embedded in the reference like `ghcr.io/org/auth-review:v1` selects the remote automatically); new `registry login <host> --token-stdin` / `registry logout` store a bearer token in the keyring (or a `0600` file fallback), with `PROMPTGENIE_REGISTRY_TOKEN` for CI. The remote backend speaks the OCI distribution API, pushes an OCI image manifest (`application/vnd.oci.image.manifest.v1+json`) carrying PromptGenie layer kinds in annotations, and **dedups blobs on push** (`HEAD` each blob, upload only the missing ones). **The server is untrusted:** every blob and manifest is verified client-side against its digest (fail-closed), so a digest pin (`@sha256:`) or `--require-signed --pubkey` gives end-to-end integrity independent of the transport or registry operator. Detached signatures use a cosign-style `sha256-<hex>.sig` tag (interoperable PromptGenie↔PromptGenie on any OCI registry). HTTPS-only (SSRF-guarded; `--insecure` opt-in for http with a warning), per-object size caps, air-gap aware (`security.airgap` blocks all remote access while the local store keeps working), and audited (login/push/pull events, tokens redacted). The local and remote stores share one interface, so `push_bundle` / `materialise` / signing are backend-agnostic. New `promptgenie/core/registry_remote.py`, `promptgenie/core/registry_backend.py`, `promptgenie/core/registry_auth.py`; `httpx` ships in the new `promptgenie[registry-remote]` extra (base install stays dependency-free; remote tests use httpx's in-process `MockTransport`, no live network). 18 tests in `tests/test_registry_remote.py`. Deferred to B.2: SSO/OIDC device-flow login + per-user attribution.
- **`promptgenie registry` — local-first prompt registry (Phase 6, Phase A).** A versioned, signed, content-addressable store for prompt artifacts with an OCI-inspired layout (`blobs/sha256/`, `manifests/sha256/`, `index.json` mapping `repo:tag → manifest digest`). A prompt artifact bundles everything needed to reproduce a run — the spec (or Markdown prompt) plus its template, policy, context files, and output schema — as content-addressed layers under one manifest, with a JSON config blob of metadata (provider/model/classification; non-vendorable glob/cmd/url context recorded as `dynamic_context`). Commands: `push <spec|prompt> [--name REPO] --tag T [--sign --key K --method minisign|cosign] [--annotate k=v]`, `pull <ref> [--out DIR] [--require-signed --pubkey K]`, `list`, `tags <repo>`, `show <ref>`, `verify <ref> --pubkey K`, `rm <ref>`, `prune [--dry-run]`, `search <query>`; all with `--format json`. References are `[host/]namespace/name[:tag][@sha256:<digest>]` (default tag `latest`; a `@sha256:` digest pin is immutable). Every blob and manifest is verified by digest on read (fail-closed); signing (over the manifest digest, which transitively covers all layers) is optional and enforceable with `--require-signed`; pull materialisation is path-traversal-guarded (rejects `..` and absolute paths). Each push/pull writes an audit event. Store defaults to `~/.local/share/promptgenie/registry`, overridable with `--store-path` or `PROMPTGENIE_REGISTRY_PATH` (e.g. an in-project `.promptgenie/registry`). Dependency-free (minisign/cosign are optional external CLIs, already used by `pack verify`); the remote/HTTPS backend is deferred to Phase B. New `promptgenie/core/registry_store.py`, `promptgenie/core/artifact.py`, `promptgenie/core/registry_signing.py`, `promptgenie/commands/registry_cmd.py`, `promptgenie/schemas/prompt-manifest.schema.json`, plus `sign_blob_file()` in `core/pack_signing.py`. 41 tests in `tests/test_prompt_registry.py`. Design: [docs/registry-design.md](docs/registry-design.md).
- **`promptgenie make` — YAML task-graph batch runner (Phase 6).** A small `make`/`just`/`Taskfile`-style runner: declare `lint` / `scan` / `test` / `evaluate` (or any shell command) as tasks in `promptgenie.make.yaml`, wire them with `needs: [...]`, and run targets in topological order. `promptgenie make [TARGET...]` (or `--target`, repeatable) runs the requested tasks and their dependencies; with no target it runs every task. `--changed` skips tasks whose `inputs:` globs (supporting `**`) did not match a changed file (`git diff <--base-ref>...HEAD`, default `origin/main`) — an aggregator like `ci` then runs only the dirty sub-tasks. `--parallel N` runs independent tasks concurrently; the default is fail-fast (a failure aborts the run) while `--keep-going` continues independent tasks (a task whose dependency failed is always skipped). `--dry-run` prints the resolved plan, `--list` lists tasks, `--format json` emits a machine-readable per-task report (status, exit code, duration, output). Tasks support multi-command `run:` lists (fail-fast within a task). Cycle and unknown-dependency detection at load. Dependency-free (stdlib only); the command runner is injectable so the scheduler is fully unit-tested. Shell execution (`shell=True`) is intentional and scoped to the author-trusted, repo-committed makefile (same trust model as a Makefile). New `promptgenie/core/make.py`, `promptgenie/commands/make_cmd.py`. 32 tests in `tests/test_make.py`.
- **`promptgenie fmt` — canonical formatter for prompts & PromptSpecs (Phase 6).** A deterministic `gofmt`/`black`-style normaliser so diffs stay focused on content, not whitespace. **Markdown** (fence-aware, byte-for-byte preservation of fenced code): trims trailing whitespace, collapses blank-line runs, normalises ATX headings (single space after `#`, drops closing hashes), pads one blank line around headings, strips leading/trailing blanks, and ensures a single final newline. **PromptSpec YAML**: the same whitespace normalisation plus a canonical key sort matching the `PromptSpec` field order (top-level and the `output_contract` / `run` sub-maps) — applied only when keys are genuinely out of order, so already-ordered files keep their styling. Comments are preserved when `ruamel.yaml` is installed (new `promptgenie[fmt]` extra); without it, commented specs keep their key order (only whitespace is normalised) so a comment is never silently dropped — base install stays dependency-free. File arguments are formatted **in place** by default (atomic write, only changed files touched); directories are walked recursively for recognised extensions; stdin (`-`) formats to stdout. `--check` writes nothing and exits 1 if any file would change (CI-safe), `--diff` prints a unified diff, `--format json` emits a machine-readable summary, `--lang auto|markdown|yaml` forces the document type. Idempotent. New `promptgenie/core/formatter.py`, `promptgenie/commands/fmt_cmd.py`. 34 tests in `tests/test_formatter.py`.
- **Structured output contracts — validate & repair.** New `promptgenie validate-output FILE --schema S` validates a model response (file or stdin) against a JSON Schema; `promptgenie output repair FILE --schema S` best-effort coerces malformed output to fit (extracts JSON embedded in prose, coerces scalar types, fills missing required fields from their `default`/type, re-validates, and lists the repairs made); and `promptgenie run --schema S --output-mode json` validates the live response against `--schema` or the spec's `output_contract.schema` (inline mapping or file path), exiting 1 if it does not conform. Validation uses the optional `jsonschema` library when installed (new `promptgenie[schema]` extra) and otherwise a built-in subset validator (`type`, `required`, `properties`, `items`, `enum`, `additionalProperties`, `minimum`/`maximum`, `minLength`/`maxLength`, `minItems`/`maxItems`, `pattern`) — base install stays dependency-free. New `promptgenie/core/output_contract.py`, `promptgenie/commands/validate_output.py`, `promptgenie/commands/output_cmd.py`. 36 tests in `tests/test_output_contract.py`.
- **`promptgenie graph` — prompt dependency graph.** New command that renders how PromptSpecs and workflows depend on their templates, target profiles, providers/models, policies, context sources, and output schemas. `promptgenie graph FILE...` graphs the given specs/workflows; with no arguments it discovers every recognisable spec/workflow under `--root`. Typed nodes (`spec`, `workflow`, `step`, `template`, `target`, `provider`, `model`, `policy`, `context`, `schema`) de-duplicate shared dependencies to single nodes, and a workflow's `depends_on` becomes step→step edges. Output `--format mermaid|dot|json` (Mermaid by default; colour-coded by node kind), `--out FILE` to write. Dependency-free (`promptgenie/core/graph.py`, `promptgenie/commands/graph_cmd.py`). 15 tests in `tests/test_graph.py`.
- **Summarisation compression — heuristic low-value-section removal.** `promptgenie compress --summarise` (alias `--summarize`) now drops whole low-value sections — examples, changelog, appendix, log dumps — on top of the in-place techniques. It splits the document into heading-delimited sections (fence-aware: `#` lines inside code blocks are ignored) and removes low-value ones together with their subsections. Without a budget it removes only low-value sections; with `--max-tokens` it additionally drops other non-protected sections (largest first) until the document fits. Protected sections (objective, scope, requirements, output/format, guardrails…) and the preamble are never dropped, and a subtree containing any protected heading is preserved. New `compress(..., summarise=True)` parameter, `prune_sections()` helper, and `CompressResult.dropped_sections` (reported in `--format json`). Fully dependency-free and deterministic — no LLM, no network. 15 tests in `tests/test_summarise_compression.py`.
...
- **`promptgenie tokens`** — a read-only token inspector and companion to `compress`/`optimize` (`promptgenie/commands/tokens_cmd.py`). Reports a prompt's token count, the active estimator (`tiktoken` or the `len/4` heuristic), the potential saving of each compression technique applied individually, and the combined default-tier and all-technique totals — without modifying anything. `--format text|json|yaml`; reads from a file or stdin. Completes the ROADMAP's Token & Cost Optimizer trio. 6 tests in `tests/test_tokens_inspector.py`.
...
- **NousResearch Hermes integration** — first-class support for the Hermes model family:
  ...
  - **Setup** — no `provider add` needed; export `NOUS_API_KEY` and go. Override the endpoint/model (OpenRouter, Together, self-hosted vLLM, …) with `provider add hermes`. Full step-by-step instructions in the README's [Hermes (NousResearch)](README.md#hermes-nousresearch) section:
    ...
    promptgenie run spec.yaml --provider hermes --model Hermes-4-405B --stream
    ```
